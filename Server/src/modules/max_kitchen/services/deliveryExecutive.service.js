...
export const getAddressIdForStop = async (stop, date, session) => {
  try {
    const mainPrisma = (await import('../../../config/prisma.js')).default;
    const stopDate = stop.date ? new Date(stop.date) : (date ? new Date(date) : null);
    if (!stopDate || isNaN(stopDate.getTime())) return null;
    ...
    const user = await mainPrisma.user.findFirst({
      where: {
        OR: [
    ...
    if (user) {
      const deliveryItem = await mainPrisma.deliveryItem.findFirst({
        where: {
          userId: user.id,
      ...
      if (deliveryItem?.addressId) return deliveryItem.addressId;
    }
    return null;
  } catch (error) {
