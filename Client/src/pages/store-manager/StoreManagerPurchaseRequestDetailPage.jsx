...
import { useKitchenInventoryMock, useKitchenPurchaseRequestManagerApi } from '../../hooks/adminHook/kitchenStoreHook';
...
const StoreManagerPurchaseRequestDetailPage = () => {
  ...
  useEffect(() => {
    ...
    setSelectedLineId(activeRequest.lines[0]?.id || '');
    setLineDecisions(
  ...
  const onResolveExisting = async (lineId) => {
    ...
    const result = await resolveRequestLineItem(requestId, lineId, {
      ...
      manager_note: form.manager_note || 'Mapped to inventory item'
    });
  ...
  const onCreateAndResolve = async (line) => {
    ...
    const resolveResult = await resolveRequestLineItem(requestId, line.id, {
      ...
      manager_note: form.manager_note || 'Created new inventory master item'
    });
    ...
    <StorePageShell>
      ...
      {detailLoading && !activeRequest ? (
        ...
        <>
          ...
          <StoreSection title="Request Lines" description="Select a row to edit the decision, note, and resolution details." tone="sky">
            <Table>
              <TableHeader>
                <TableRow>
          ...
          {selectedLine ? (
            <StoreSection
              title={`Selected Line: ${selectedLine.requested_item_name || selectedLine.inventory_item_name || 'Unnamed line'}`}
              description={`${selectedLine.requested_quantity} ${selectedLine.requested_unit}`}
              tone="emerald"
            >
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <div className="space-y-3 rounded-md border p-4">
                  {selectedLine.is_new_item ? (
                    <>
                      <p className="text-sm font-medium">Create Inventory Item and Resolve</p>
