...
export function useJaiceGreeting(options = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/ml-assistant/greeting');
      return data;
    },
    staleTime: 5 * 60 * 1000,
    ...options,
...
export function useJaicePing(options = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/ml-assistant/chat/ping');
      return data;
    },
    staleTime: 30 * 1000,
    ...options,
...
export function useJaiceDebugConfig(options = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/ml-assistant/debug/config');
      return data;
    },
    staleTime: 60 * 1000,
    ...options,
...
export function useJaiceChat(options = {}) {
  ...
  return useMutation({
    mutationFn: async ({ messages, max_tokens = 512, temperature = 0.3 }) => {
      const { data } = await api.post('/ml-assistant/chat', {
        messages,
        max_tokens,
        temperature,
      });
